...
HRESULT RacorX2::ConfirmDevice(D3DCAPS8* pCaps, DWORD dwBehavior, D3DFORMAT Format)
{
...
HRESULT RacorX2::OneTimeSceneInit()
{
...
HRESULT RacorX2::InitDeviceObjects()
{
	HRESULT hr;
...
HRESULT RacorX2::RestoreDeviceObjects()
{
	HRESULT hr;
	...
	ID3DXBuffer *pBuffer;
	ID3DXBuffer *pError;
	hr = D3DXAssembleShader(vsh, sizeof(vsh) - 1, 0, NULL, &pBuffer, &pError);
	if (FAILED(hr)) {
		MessageBox(m_hWnd, L"D3DXAssembleShader failed!", L"Error", 0);
		return E_FAIL;
	}
	hr = m_spDevice->CreateVertexShader(dwDecl, (DWORD*)pBuffer->GetBufferPointer(), &m_dwVSH, 0);
	if (FAILED(hr)) {
		MessageBox(m_hWnd, L"CreateVertexShader failed!", L"Error", 0);
		return E_FAIL;
	}
	*/
	hr = CreateVSFromBinFile(m_spDevice.get(), dwDecl, L"basic.vso", &m_dwVSH);
	...
	m_spDevice->SetVertexShaderConstant(4, material, 1);

	return S_OK;
}

HRESULT RacorX2::DeleteDeviceObjects()
{
	
	return S_OK;
}

HRESULT RacorX2::Render()
{
	D3DXMATRIX matTem;
	matTem = m_mtWorld * m_mtView * m_mtProj;
	D3DXMatrixTranspose(&matTem, &matTem);
	...
	if (SUCCEEDED(m_spDevice->BeginScene())) {
		...
		m_spDevice->SetVertexShaderConstant(0, &matTem, 4);
...
HRESULT RacorX2::FrameMove(FLOAT)
{
	return S_OK;
}

HRESULT RacorX2::FinalCleanup()
{
	return S_OK;
}

const DWORD Vertex::FVF = D3DFVF_DIFFUSE;
