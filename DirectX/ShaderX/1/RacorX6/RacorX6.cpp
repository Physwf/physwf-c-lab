...
HRESULT RacorX6::OneTimeSceneInit()
{
	...
	D3DXMatrixPerspectiveFovLH(&m_mtProj, D3DX_PI*0.5, static_cast<float>(m_iWidth) / m_iHeight, 1.0, 1000.0f);
...
HRESULT RacorX6::RestoreDeviceObjects()
{
	...
	if (m_bPS14Avaliable)
	{
		hr = CreatePSFromBinFile(m_spDevice.get(), L"diff14.pso", &m_dwPSH14);
		if (FAILED(hr))
		{
			MessageBox(m_hWnd, L"CreatePSFromBinFile failed!", L"Error", 0);
			return E_FAIL;
	...
	D3DXVECTOR4 vLight(1, 0, 0,0);
	m_spDevice->SetVertexShaderConstant(LIGHT_DIR, &vLight, 1);
...
{
	m_spDevice->Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x000000ff, 1.0f, 0);
	if (SUCCEEDED(m_spDevice->BeginScene())) {
		m_spDevice->SetTexture(0, m_spColorMap.get());
		m_spDevice->SetTexture(1, m_spNormalMap.get());
		m_spDevice->SetVertexShader(m_dwVSH);
		m_spDevice->SetPixelShader(m_dwPSH);
		m_spEarthMesh->DrawSubset(0);
		m_spDevice->EndScene();
	}
...
HRESULT RacorX6::FrameMove(FLOAT delta)
{
	...
	m_spDevice->SetVertexShaderConstant(CLIP_MATRIX, &matTemp, 4);
	
	D3DXMatrixTranspose(&matTemp, &m_mtWorld);
	m_spDevice->SetVertexShaderConstant(TRASPOSED_WORLD_MATRIX, &matTemp, 4);
