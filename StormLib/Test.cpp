...
static int TestFileExtraction(HANDLE hMpq1, HANDLE hMpq2, const char * szListFile, DWORD dwBlockSize, BOOL bDoRandomRead = FALSE)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        clreol();
        printf("Extracting files\r");

        // Go through all the files found in MPQ
        while(hFind != NULL && bResult == TRUE)
        {
            // Test only if not a list file
            if(_stricmp(wf.cFileName, LISTFILE_NAME))
            {
                ...
                // Debug
//              if(!_stricmp(wf.cFileName, "monsters\\mega\\megas2.wav"))
...
static int TestMpqFindAndReadFile(const char * szMpqName, const char * szListFile)
{
    SFILE_FIND_DATA sf;
    LPVOID pbBuffer = NULL;
    HANDLE hFind = NULL;
    HANDLE hFile = NULL;               // Archive file open by StormLib
    HANDLE hMpq = NULL;
    DWORD dwBlockSize = 10000;
    BOOL bResult = TRUE;
    int nError = ERROR_SUCCESS;
    ...
    {
        clreol(); printf("Allocating blocks ...\r");
        pbBuffer = (BYTE *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, dwBlockSize);
        if(pbBuffer == NULL)
            return ERROR_NOT_ENOUGH_MEMORY;
    }
    ...
    // Search all files and read their content
    if(nError == ERROR_SUCCESS)
    {
        ...
        while(hFind != NULL && bResult == TRUE)
        {
            ...
            // Show the file being tested
            ShowProcessedFile(sf.cFileName);

            // Try to open with Storm.dll and StormLib
            bResult = SFileOpenFileEx(hMpq, sf.cFileName, 0, &hFile);
            if(bResult == TRUE)
            {
                DWORD dwFileFlags = SFileGetFileInfo(hFile, SFILE_INFO_FLAGS);
                DWORD dwBytesRead = 1;

                // Debug
                if((dwFileFlags & MPQ_FILE_02000000) && sf.dwFileSize != 1)
                {
                    _asm int 3;
                }
                
                if((dwFileFlags & MPQ_FILE_02000000) == 0 && sf.dwFileSize == 1)
                {
                    _asm int 3;
                }
                ...
                while(dwBytesRead != 0)
                {
                    if(!SFileReadFile(hFile, pbBuffer, dwBlockSize, &dwBytesRead, NULL))
                    {
                        if(GetLastError() != ERROR_HANDLE_EOF)
                            printf("Error reading \"%s\" (code %u)\n", sf.cFileName, GetLastError());
                    }

                    dwTotalRead += dwBytesRead;
                }

                if(dwTotalRead != sf.dwFileSize)
                    printf("File size mismatch \"%s\"\n", sf.cFileName);
...
//  if(nError == ERROR_SUCCESS)
//      nError = TestMpqFindAndReadFile(szMpqName, szListFile);
...
//      nError = TestMpqRenaming(szMpqCopyName, szListFile);
    ...
    if(nError == ERROR_SUCCESS)
        nError = TestNamelessAccess(szMpqName);
