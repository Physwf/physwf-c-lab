...
*   requires Carbon framework/CarbonLib
*
...
static OSErr FSGetFullPath(const FSRef *ref, UInt8 *fullPath, UInt32 fullPathLength)
{
    ...
    return result;
#else
    // Hopefully this works, but if there is still somebody out there that 
    // develops for pre OS X, they'll need to debug it, as I can't
    OSErr           result;
